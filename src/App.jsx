...
export function App(){
  const [notaAvaliacao, setNotaAvalaicao] = useState(0)
  const [submited, setSubmited] = useState(false)

  

  function handleMudarNotaAvaliacao(nota){
    setNotaAvalaicao(nota)
  }
  ...
  return(
    submited === false ?(
      <div className="bg-gradient-dark text-white mx-6 p-6 rounded-2xl font-overpass max-w-103">
        ...
        <button onClick={handleSubmit} className="bg-orange w-full uppercase trackgin-1 font-bold rounded-3xl text-sm py-3 hover:bg-white hover:text-orange cursor-pointer">Submit</button>
    </div>
    ) : (
      <div className="bg-gradient-dark text-white mx-6 p-6 rounded-2xl font-overpass max-w-103">
